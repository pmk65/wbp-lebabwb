 ...
 *
 *     61   function OnReady()
 *     75   function OnDisabled()
 *     88   function OnExit()
 *    100   function ToggleDock(Sender)
 *    117   function D(val)
 *    129   function CreateTabPanel(parent, caption)
 *    150   function CreateDockingPanel()
 *    465   function CreateCheckBox(parent, caption, hint, tagId, tabOrder, left, top)
 *    494   function CreateTranspileButton(parent, caption, tagId, left, top)
 *    518   function SaveCheckboxState(Sender)
 *    531   function GetSettings(tagId)
 *    553   function StartTranspiling(Sender)
 *    618   function InstallGlobalNodeModule(module)
 *    628   function GetEditorData()
 *    651   function SelectTagBlock(tag)
 *    697   function DisplayWarnings(text, editorStartLine)
 *    710   function ScaleImage(NewWidth, NewHeight)
 *    738   function FileGetContents(filename)
 *    758   function FilePutContents(filename, contents)
 *    778   function DeleteFile(file, force)
 *    794   function ExpandEnviromentVariable(path)
 *    806   function GetTempFileName(ext)
 *    819   function OpenUrlInBrowser(url)
 *    829   function OnInstalled()
 *
 * TOTAL FUNCTIONS: 24
